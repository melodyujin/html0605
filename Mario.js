class Mario extends GameObject {
    //주인공은 이미지를 둘러싼 wrapper와
    //4개의 센서막대도 있어야 한다.
    //js에서 개발자가 생성자를 정의하지 않을 경우
    //js 자체적으로 부모를 먼저 생성해준다
    //하지만 개발자가 현재 클래스의 생성자를 정의할 경우
    //개발자가 주도하는 것이기에 js에서 부모를 자동 생성해주지 않으므로,
    //개발자가 부모생성처리까지 맡아야 한다
    ...
    //부모인 GameObject의 메서드가 마리오에 적용하기엔
